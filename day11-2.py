...
def mark_until(map, counts, row, col, x_dir, y_dir):
    ...
    while next_x >= 0 and next_x < len(map) \
      and next_y >= 0 and next_y < len(map[row]):
        # print(next_x, next_y, len(map[0]), len(map))
        counts[next_x][next_y] += 1

        # if it's a seat, we should increment, but break
        if map[next_x][next_y] in [OCCUPIED, EMPTY_SEAT]:
            break
